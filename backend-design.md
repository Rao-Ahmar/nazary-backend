...
**Computed**:
- `seats_left` → `total_seats - bookings.confirmed.sum(:seats)`
...
### 5.9 Categories & Collections (Public — No Auth)
...
### Categories Controller (Public — no auth)
...
module Api
  module V1
    class CategoriesController < BaseController
      ...
      def index
        render json: { data: Category.all.map { |c| { id: c.id.to_s, label: c.label, icon: c.icon } } }
      end
    end
  end
end
...
### Collections Controller (Public — no auth)
...
module Api
  module V1
    class CollectionsController < BaseController
      ...
      def index
        render json: { data: Collection.all.map { |c|
          {
            id: c.id.to_s,
            title: c.title,
            subtitle: c.subtitle,
            image: c.cover_image.attached? ? url_for(c.cover_image) : nil
          }
        }}
      end
      ...
      def show
        ...
        render json: {
          ...
          image: collection.cover_image.attached? ? url_for(collection.cover_image) : nil,
          trips: collection.trips.active.map { |t| TripListSerializer.new(t).as_json }
        }
      end
    end
  end
